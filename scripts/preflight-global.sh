...
[ "$(grep -c 'pattern = ' "$CFG")" = 9 ] && ! grep -Eq 'pattern = "[^"]*(cart|checkout|store_sessions|\?)' "$CFG" && ! grep -q 'pattern = "www.usesul.com.br/usesul/\*"' "$CFG" && grep -q 'pattern = "www.usesul.com.br/usesul\*"' "$CFG" && grep -q 'pattern = "www.usesul.com.br/usesul/"' "$CFG" && grep -q 'www.usesul.com.br/usesul/product/\*' "$CFG" && grep -q 'www.usesul.com.br/__origens/\*' "$CFG" && ok "rotas do TOML (9, todas COM Worker): produto, __origens, /usesul*, home (/usesul e /usesul/) e as de casca; nenhuma de carrinho/checkout/login, nenhuma com query; a exclusão /usesul/* NÃO está no TOML (zona: scripts/zone-routes.mjs)" || bad "rotas do TOML diferentes das esperadas: $ROUTES"
if RSTAGE=$(for st in baseline staged final; do node scripts/zone-routes.mjs verify --stage=$st >/dev/null 2>&1 && echo $st && break; done) && [ -n "$RSTAGE" ]; then ok "rotas REAIS da zona (API, somente leitura): estágio '$RSTAGE'"; else warn "rotas reais da zona não conferidas (sem credencial da Cloudflare ou estado inesperado): node scripts/zone-routes.mjs list"; fi
