...
const G = () => page.evaluate(() => {
  ...
  const st0 = main.scrollTop; // esconder o bloco encolhe o painel e o navegador zera o scroll: a medição não pode alterar o estado observado
  if (root) root.style.display = 'none'; const w0 = hs(); const wItem0 = wI(); if (root) root.style.display = ''; main.scrollTop = st0;
...
if (!LIVE) {
  // O espelho é gravado SOB DEMANDA: só ao SAIR por um link nosso. Lê o DOM da INK primeiro, sai (storefront de teste) e só então lê o snapshot.
  await page.route('https://useorigens.com.br/**', (route) => route.fulfill({ status: 200, contentType: 'text/html', body: '<html><body>storefront stub</body></html>' }));
  ...
  const writesBefore = refs.length;
  await Promise.all([page.waitForURL(/useorigens\.com\.br\/sul/, { timeout: 15000 }).catch(() => null), page.locator('.cart-drawer [data-origens-discovery="cart"] .o-cta').click({ noWaitAfter: true })]); await wait(1200);
  check('espelho sob demanda: a saída pelo link nosso criou 1 snapshot (e nada foi gravado antes)', writesBefore === 0 && refs.length === 1, 'antes ' + writesBefore + ', depois ' + refs.length);
  const ref = refs[refs.length - 1]; const snap = ref ? await (await mf.dispatchFetch(HOST + '/__origens/cart-ref/' + ref)).json() : null;
