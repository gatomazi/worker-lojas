...
- **Desktop:** logo (→ `useorigens.com.br/sul`), até 5 coleções + "Mais", "Cidades" (→ `/sul#estados`), lupa. **Mobile:** hambúrguer, logo (só a imagem, 48 px, centralizada na faixa; o texto "Use Origens" fica só para leitores de tela), lupa e carrinho; coleções e "Cidades" no topo do menu lateral nativo (a "Categorias" nativa some só quando as nossas existem).
